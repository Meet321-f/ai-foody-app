...
export default function CreateRecipe() {
  ...
  const onGenerate = async () => {
    ...
    try {
      ...
      const result = await GlobalApi.AiModel(prompt, token);
      const recipeText =
        result?.choices?.[0]?.message?.content || "No recipe generated.";
      ...
      try {
        ...
        const jsonMatch =
          cleanText.match(/\[.*\]/s) || cleanText.match(/\{.*\}/s);
  ...
  const GenerateCompleteRecipe = async (option: any) => {
    ...
    try {
      ...
      const imageUrl = await GenerateRecipeImage(
        JSONContent?.imagePrompt || option.recipeName,
      );
  ...
  const GenerateRecipeImage = async (imagePrompt: string) => {
    try {
      const token = (await getToken()) || "";
      const result = await GlobalApi.GenerateAiImage(imagePrompt, token);
        ...
        <BlurView intensity={60} tint="light" style={styles.glassOverlay}>
          <View style={styles.glassContent}>
            <View style={styles.cardContainer}>
              <Image
                source={require("../assets/images/pen.gif")}
              ...
              <Text style={styles.subheading}>
                Make something for your LOVE! ❤️
              </Text>
              ...
              {userInput ? (
                <Text style={styles.previewText}>Input: {userInput}</Text>
              ) : null}
